...
> **Activity and nutrition are intentionally decoupled.** Movement is logged for its own sake (life balance, reflection, the movement pillar) — it does *not* adjust your nutrient targets or intake. Earlier builds estimated nutrient effects from activity (e.g. magnesium/B-vitamin loss from exertion, vitamin D from being outdoors) using duration alone, which is too coarse to be trustworthy. Vitamin D is the one exception that *is* still credited, because it can be estimated from sun exposure using skin type, exposed body surface, and UV — independent of how hard you moved.
>
> **Concept — movement → nutrition via wearables (future).** Coupling movement back to nutrition is on the roadmap, but only once it can be done accurately. The planned wrist band (IMU + skin temperature + PPG) gives the missing signals — heart rate / HRV for true energy expenditure and sweat-driven electrolyte loss, skin/body temperature for thermal load — so depletion of magnesium, sodium, potassium, and B-vitamins can be modeled from measured physiology instead of guessed from a label like "ran 30 min." Until the wearable is built and validated against real data, this stays a concept and the activity↔nutrition link remains off.
