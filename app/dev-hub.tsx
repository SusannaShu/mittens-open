...
const STORAGE_KEY_TUNNEL = 'devhub_tunnel_url';
const STORAGE_KEY_LAN = 'devhub_lan_url';
const DEV_HUB_LOCAL = 'http://localhost:4000';
...
export default function DevHubScreen() {
  ...
  const discoverUrl = useCallback(async (overrideUrl?: string) => {
    ...
    if (__DEV__) {
      ...
      if (debuggerHost) {
        ...
        const expoLanUrl = `http://${host}:4000`;
    ...
    setLoading(false);
    setError(
      'Cannot reach Dev Hub.\n\nMake sure the server is running (npm run dev in dev-hub) and tunnel is enabled for remote access.'
    );
  }, []);
  ...
  const handleError = useCallback(() => {
    setLoading(false);
    setError('Connection lost. Dev Hub may have restarted.');
  }, []);
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: Math.max(insets.top, Platform.OS === 'ios' ? 40 : 20) }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Dev Hub</Text>
