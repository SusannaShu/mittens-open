/**
 * GeminiCloudProvider -- wraps existing Backend API calls to conform
...
export class GeminiCloudProvider implements InferenceProvider {
  async identifyFoods(images: string[], caption?: string): Promise<FoodIdentification> {
    // Uses the existing smart-snap endpoint which routes through Backend -> Gemini
    if (images.length === 0) {
      return { foods: [] };
    }
    ...
    // Cloudinary / the Backend backend expect properly data-prefixed base64 strings
  ...
  async estimateNutrients(
    ...
    // The Backend backend already handles this in the nutrition-log controller.
    ...
    return {
      ...
      meta: {
        ...
        allRefs: [],
        adjustments: [],
        justification: 'Cloud estimation via Backend -> Gemini',
      },
    };
  }
